...
def output(v_xs):
    ...
    n,m = shape(v_xs)
    _pre_max = tf.reduce_max(prediction, 1)
    _result = tf.argmax(prediction, 1)
    with open("result.csv",'w', newline='') as myFile:
        myWriter = csv.writer(myFile)
        myWriter.writerow(['ImageId', 'Label'])
        for i in range(280):
            batch_xs = v_xs[i*100 : (i+1)*100]
            pre_max = sess.run(_pre_max, feed_dict={xs: batch_xs, keep_prob: 1, keep_prob_conv: 1})
            result = sess.run(_result, feed_dict={xs: batch_xs, keep_prob: 1, keep_prob_conv: 1})
            for j in range(100):
                if pre_max[j] < 0.99:
                    paint(v_xs[i*100+j])
                    _in = input(str(i*100+j)+" "+str(pre_max[j])+" "+str(result[j])+"\nWhat is this?\n")
                    if _in != '':
                        result[j] = int(_in)
            for j in range(100):
